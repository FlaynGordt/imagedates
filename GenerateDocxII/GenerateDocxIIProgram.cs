...
namespace GenerateDocxII
{
    ...
    internal class GenerateDocxIIProgram
    {
        ...
        private static void WriteDoc()
        {
            ...
            try
            {
                ...
                Console.WriteLine("Locations: " + LocationsFinder.Locations.Count);
                ...
                foreach (XElement element in xdoc.Root.Elements())
                {
                    ...
                    var locations = dates.SelectMany(d =>  LocationsFinder.FindByDateNotTime(d)).ToList();

                    posts.Add(new Post() { Content = content, Dates = dates, Title = title, Author = author, Locations = locations });
                }
                var pathToDoc = Path.Combine(Paths.SLIDY_OUT, "test.docx");
                using (var document = DocX.Create(pathToDoc))
                {
                    ...
                    var p = new Process();
                    p.StartInfo = new ProcessStartInfo(@"C:\Program Files\Microsoft Office\root\Office16\WINWORD.EXE")
                    {
                        Arguments = pathToDoc
                    };
                    p.Start();
